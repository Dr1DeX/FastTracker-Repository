...
const ModalPhoto = (props) => {
    ...
    return(
        <>
            <img onClick={toggle} src={API_STATIC_MEDIA + props.executor.photo} alt='loading' style={{height: 50}} />
            <Modal isOpen={visible} toggle={toggle}>
                ...
                <ModalBody style={{display: 'flex', justifyContent: 'center', backgroundColor: '#212529'}}>
                    <img src={API_STATIC_MEDIA + props.executor.photo} alt='loading'/>
                </ModalBody>
                <ModalFooter style={{display: 'flex', justifyContent: 'center', backgroundColor: '#212529'}}>
                    <Button type='button' onClick={() => toggle()}>Закрыть</Button>
                </ModalFooter>
            </Modal>
        </>
    )
